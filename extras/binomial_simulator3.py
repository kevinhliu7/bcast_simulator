...
final_time = 0.0
# checks if two processes are in same node
temp = [i % (PPN * N) for i in range(2, PPN * N + 2)]
topology = [list(c) for c in mit.divide(N, temp)]
# print(topology)
def in_same_node(src, dst):
    for x in topology:
        if src in x and dst in x:
            return True
    return False
...
def get_alpha_beta(src, dst):
    # src_id = src // PPN
    # dst_id = dst // PPN
    if (in_same_node(src, dst)): # checks if they are on the same node
